...
import type { ChatState, Chat, Message, User, ContactRequest, CallState } from "../types";
...
type ChatAction =
  ...
  | { type: "INITIATE_CALL"; payload: { callId: string; callee: User; callType: "audio" | "video"; user: User } }
  ...
  | {
      type: "SET_CALL";
      payload: { callId: string; callType: "audio" | "video"; status: CallState["status"] };
    };
...
export function ChatProvider({ children }: { children: React.ReactNode }) {
  ...
  const processQueuedIceCandidates = useCallback(async (peerConnection: RTCPeerConnection) => {
  ...
  pc.onicecandidate = (event) => {
    if (event.candidate && socketRef.current) {
      ...
      const currentCall = stateRef.current.call;
      const recipientId = currentCall.caller?.id === userRef.current?.id 
  ...
  async (callee: User, callType: "audio" | "video") => {
    ...
    try {
      ...
      console.log("📤 Created and set local offer with constraints:", offerOptions);
  ...
  async (callId: string) => {
    ...
    if (!socketRef.current || !currentCall.peerConnection || !currentCall.caller || !currentUser) {
  ...
  const endCall = useCallback(
    (callId: string) => {
      ...
      const recipientId = currentCall.caller?.id === currentUser.id 
        ? currentCall.callee?.id 
        : currentCall.caller?.id;
      
      const recipientPhone = currentCall.caller?.id === currentUser.id 
...
socket.on("call-accepted", async (data) => {
  ...
  //call timeout logic
  useEffect(() => {
    if (state.call.status === "ringing" && state.call.callId && socketRef.current) {
