...
function CallModal() {
  ...
  const { isVideoEnabled, isAudioEnabled, audioUnlockNeeded, toggleVideo, toggleAudio } =
    useCallMedia({ call, localVideoRef, remoteVideoRef, localAudioRef, hiddenRemoteAudioRef });

  const { position: pipPos, size: pipSize, startDrag, startResize } = useCallPip();
  ...
  useEffect(() => {
    ...
    const reset = () => {
      ...
      controlsTimerRef.current = setTimeout(() => setShowControls(false), 3_000);
    };
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      switch (e.key.toLowerCase()) {
        case "m": e.preventDefault(); toggleAudio(); break;
        case "v": if (call.callType === "video") { e.preventDefault(); toggleVideo(); } break;
        case "f": e.preventDefault(); setIsFullscreen((p) => !p); break;
        case "s": e.preventDefault(); setShowSettings((p) => !p); break;
        case "t": if (call.callType === "video") { e.preventDefault(); setVideoLayout((p) => p === "remote-main" ? "local-main" : "remote-main"); } break;
        case "escape":
          e.preventDefault();
          ...
          break;
        case "q":
          if ((e.ctrlKey || e.metaKey) && call.callId) { e.preventDefault(); endCall(call.callId); }
          break;
      }
    };
  ...
  }, [call.status, call.callType, call.callId, isFullscreen, showSettings, toggleAudio, toggleVideo, endCall]);
  ...
  return (
    ...
    >
      ...
      <audio ref={hiddenRemoteAudioRef} autoPlay playsInline style={{ display: "none" }} />
      ...
      >
        ...
        {call.status === "calling" && (
          <div className="p-8 text-center">
            <CallAvatar avatarUrl={otherUser?.avatarUrl} displayName={otherUser?.displayName} size="md" />
            <h2 className="text-2xl font-semibold mt-4 mb-2">Calling...</h2>
            <p className="text-xl text-gray-300">{otherUser?.displayName}</p>
            <p className="text-sm text-gray-400 mt-1">{otherUser?.phoneNumber}</p>
            <button
        ...
        {call.status === "ringing" && (
          <div className="p-8 text-center">
            <CallAvatar avatarUrl={otherUser?.avatarUrl} displayName={otherUser?.displayName} size="md" pulsing />
        ...
        {call.status === "connected" && (
          <div className={`relative ${isFullscreen ? "h-screen" : "h-96"} flex flex-col`}>
            {/* Header */}
            <div
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <CallAvatar avatarUrl={otherUser?.avatarUrl} displayName={otherUser?.displayName} size="sm" />
                  <div>
            ...
            {call.callType === "video" ? (
              <div className="call-container flex-1 relative">
                {/* Main video */}
                <div className="w-full h-full cursor-pointer" onClick={toggleVideoLayout} title="Switch videos (T)">
                ...
                <div
                  ...
                  style={{ left: pipPos.x, top: pipPos.y, width: pipSize.width, height: pipSize.height }}
                  onMouseDown={startDrag}
                  onTouchStart={startDrag}
                  onClick={(e) => { e.stopPropagation(); toggleVideoLayout(); }}
                >
                  {videoLayout === "remote-main" ? (
                    isVideoEnabled && call.localStream ? (
              ...
              <div className="flex-1 flex items-center justify-center bg-gradient-to-br from-gray-800 to-gray-900">
                <div className="text-center">
                  <CallAvatar avatarUrl={otherUser?.avatarUrl} displayName={otherUser?.displayName} size="md" />
                  <h3 className="text-xl font-semibold mt-4 mb-2">{otherUser?.displayName}</h3>
        ...
        {/* ── End states ──────────────────────────────────────────────── */}
        {(call.status === "declined" || call.status === "missed" || call.status === "ended") && (
          <div className="p-8 text-center">
            <CallAvatar avatarUrl={otherUser?.avatarUrl} displayName={otherUser?.displayName} size="sm" />
            <h3 className="text-xl font-semibold text-gray-300 mt-4">
              {call.status === "declined" && "Call Declined"}
              {call.status === "missed" && `Missed Call from ${otherUser?.displayName}`}
